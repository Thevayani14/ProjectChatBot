...
def get_conversation(conversation_id):
    sql = "SELECT id, user_id, title FROM conversations WHERE id = %s"
    with closing(connect_db()) as db, closing(db.cursor()) as cursor:
        cursor.execute(sql, (conversation_id,))
        data = cursor.fetchone()
        if data:
            return {"id": data[0], "user_id": data[1], "title": data[2]}
        return None
...
def log_schedule_generation(user_id, model, inference_time, was_saved):
    """Logs the result of a user-chosen schedule generation event."""
    sql = """
    INSERT INTO schedule_generation_logs 
        (user_id, model_used, inference_time_seconds, was_schedule_saved)
    VALUES (%s, %s, %s, %s)
    """
    with closing(connect_db()) as db:
        if db is None: return False
        with closing(db.cursor()) as cursor:
            cursor.execute(sql, (user_id, model, inference_time, was_saved))
            db.commit()
    return True

def log_phq9_accuracy_test(user_id, conv_id, answers, score, rule_severity, llm_severity, is_match, inference_time):
    """Logs the results of a PHQ-9 shadow accuracy test."""
    sql = """
    INSERT INTO phq9_accuracy_logs 
        (user_id, conversation_id, phq9_answers, total_score, rule_based_severity, 
         llm_generated_severity, is_match, inference_time_seconds)
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    with closing(connect_db()) as db:
        if db is None: return
        with closing(db.cursor()) as cursor:
            cursor.execute(sql, (user_id, conv_id, answers, score, rule_severity, llm_severity, is_match, inference_time))
            db.commit()