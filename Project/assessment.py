...
def initialize_assessment_session():
    # We only initialize states relevant to the assessment
...
def generate_chat_title(first_user_message, first_ai_response, model):
    prompt = f"""Summarize this exchange into a 5-word-or-less title: User: "{first_user_message}" Assistant: "{first_ai_response}" """
    try:
...
def assessment_page():
    ...
    # Create a new conversation if one isn't active for this session
    if st.session_state.assessment_conversation_id is None:
...
def display_messages():
    ...
    if not st.session_state.assessment_messages:
...
def show_results():
    ...
    # Store result for other pages to use
